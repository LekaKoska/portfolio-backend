@foreach($profile as $user)
    <section class="portfolio-card">
        <div class="flex flex-col items-center text-center">
            <a class="portfolio-avatar-wrap" href="{{ $user->profile_img ?? asset('images/avatar.jpg') }}" target="_blank" aria-label="Open avatar">
                <img
                    src="{{ $user->profile_img ?? asset('images/avatar.jpg') }}"
                    alt="{{ $user->name }}"
        ...
        <div class="mt-5 flex flex-wrap gap-2">
            ...
            @if($user->cv_pdf)
                <a class="portfolio-linkchip portfolio-linkchip--primary" href="{{ $user->cv_pdf }}" target="_blank">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
